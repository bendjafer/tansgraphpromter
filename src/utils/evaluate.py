...
def _get_accuracy_airport(eval_output, path):
    """Shared eval for all airport datasets (Low/Medium/High/Very High)."""
    df = pd.concat([pd.DataFrame(d) for d in eval_output])
    with open(path, 'w') as f:
        for _, row in df.iterrows():
            f.write(json.dumps(dict(row)) + '\n')
    # "Very High" must come before "High" so the longer match wins
    classes_regex = r'(Very High|High|Medium|Low)'
    correct = sum(
        1 for pred, label in zip(df['pred'], df['label'])
        if re.findall(classes_regex, pred) and re.findall(classes_regex, pred)[0] == label
    )
    return correct / len(df)


def get_accuracy_brazil(eval_output, path):
    return _get_accuracy_airport(eval_output, path)


def get_accuracy_europe(eval_output, path):
    return _get_accuracy_airport(eval_output, path)
